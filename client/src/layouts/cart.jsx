...
const handleRemoveItem = async (productId, cartItemId) => {
    ...
    return (
        <div className="h-screen pt-20">
            <h1 className="text-center text-2xl font-bold mb-8">Cart Items {countTotalItems()}</h1>
            {cartItems.length === 0 ? (
                ...
                <div className="mx-auto max-w-5xl px-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div className="md:col-span-2 space-y-8">
                            {cartItems.map((item) => (
                                <div key={item.cart_item_id} className="bg-white rounded-lg p-6 shadow-md flex items-start">
                                    <img
                                    ...
                                    <div className="flex-grow flex justify-between">
                                        ...
                                        <div className="flex items-center flex-col mt-4">
                                            ...
                                            <button className="px-3 py-1 bg-gray-200 text-gray-700 hover:bg-gray-300" onClick={() => {
    if (item.quantity > 1) {
        handleQuantityChange(item.cart_item_id, item.quantity - 1);
    }
}}>-</button>
                                                <input className="w-16 px-2 py-1 text-center text-sm text-gray-700" type="text" min="1" value={item.quantity} readOnly  />
                                                <button className="px-3 py-1 bg-gray-200 text-gray-700 hover:bg-gray-300" onClick={() => handleQuantityChange(item.cart_item_id, item.quantity + 1)}>+</button>
                                            </div>
                                            <p className="ml-4 text-gray-800 mt-4">Price: {numberWithCommas(item.product.price)} B</p>
                                        </div>
                                    </div>
                                    <button className="ml-4 text-red-500" onClick={() => handleRemoveItem(item.product.id, item.cart_item_id)}>Remove</button>
                                </div>
                            ))}
                        </div>
                        <div className="bg-white rounded-lg p-6 shadow-md">
                            <div className="flex flex-col">
                                <div className="flex justify-between mb-4">
                                    ...
                                    <p className="text-gray-700">{numberWithCommas(calculateTotalPrice())} B</p>
                                </div>
                                <div className="flex justify-between mb-4">
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <div>
                                        <p className="text-lg font-bold">{numberWithCommas(parseFloat(calculateTotalPrice()) + 50)} B</p>
                                    </div>
                                </div>
